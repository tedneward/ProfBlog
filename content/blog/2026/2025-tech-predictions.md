...
***Result***: As I write this (on December 31st, 2025), I can only say that we got *so* close. But we didn't get it slid in under the wire--we're still iterating on proofs with the publisher. That said, we do have a [publisher page](https://link.springer.com/book/9798868818943), and we can take pre-orders, and....
...
#### "Developers are obsolete" prose will peak, then dwindle (*p* 0.8)
...
But the complete collapse of a startup isn't the only way it fails; Google, for example, did a [talent-acquisition play](https://www.joinpavilion.com/blog/what-windsurfs-exit-says-about-startup-culture-in-2025) with Windsurf, which raises the question, "If Windsurf fails in 2026, is that because the market isn't there, or because the company didn't execute, or because the company's [key executors were scooped out](https://jomasego.medium.com/what-happened-to-windsurf-20297b7d9b14)?"
...
***Result***: It's hard to say, honestly. Lots of sample and demo code has sprung up using type hinting, and the coding-assistant tools love to slither it in to the code they generate, but are Pythonistas actually embracing it? Well, [Meta thinks so](https://engineering.fb.com/2025/12/22/developer-tools/python-typing-survey-2025-code-quality-flexibility-typing-adoption/). [Pyrefly agrees](https://pyrefly.org/blog/why-typed-python/). And hey, if [Reddit agrees](https://www.reddit.com/r/Python/comments/1nti5ar/why_todays_python_developers_are_embracing_type/), it must be true, right?
...
***Result***: **+1**. For starters, they took it to production status; for seconders, their blog was filled with comparisons to React Native and the iOS or Android native ecosystems. And, in some ways, it appears to have made something of an impact, as their own [developer survey](https://www.snappmobile.io/kmp) suggests that there's a non-trivial number of folks using it. (They survey was for 2024, but the analysis came out in 2025, so we have to take the data with at least a little salt.)
...
***Result***: Ohboy. *THIS* one. I was pretty confident about this one, until... [boom](https://thenewstack.io/microsoft-goes-all-in-on-rust-for-core-infrastructure-and-much-more/) and [boom](https://thenewstack.io/microsofts-rust-bet-from-blue-screens-to-safer-code/). Windows is a massive C/C++ codebase (though there's some other stuff in there too, to be sure), so if they're starting to replace the C/C++ code with Rust code, that's a pretty solid vote of confidence.
...
#### 2026 AI predictions are going to be even wilder than 2025's (*p* 0.8)
...
* Somebody, typically one of the folks with close ties to venture capital or Y Combinator, comes up with an idea. It doesn't have to be an actually accomplishable idea, it just has to be able to look remotely feasible and have some kind of sexy demo.
* The founder, through the VC's network, gets some tech press folks to do some interviews.
...
* Other VCs, now desperate to jump on to a "winning thing", immediately pony up some money.
* This reinforces the press coverage, as other tech press outlets immediately clamor for interviews. Trusting that the scrutiny has already been done ("I mean, if you're a VC, you wouldn't just put umpteen-million into it without doing a serious due diligence on it, right?"), they offer up even softer-ball interviews, and the deification takes root. This is "The Next Big Thing".

Unfortunately, as Theranos showed us, this basically has a predictable cycle:

* The CEO/founder does a lot of press, and makes wild-eyed promises that the tech press eats up.
* As deadlines approach, the CEO soft-pedals back some of the promises, but makes other, lateral, promises as a way of showing how "on top of things" the company really is. "We're so close" becomes a common refrain, along with "We just need... to get there."
* The company maybe IPOs, if it can't swing sweetheart deals with massive backers (lookin' at you, Microsoft, Google, Amazon...).
* With each missed deadline, the CEO grows wilder in their predictions, and the needs grow more outlandish.
* At some point in all this, somebody gets an uneasy "Waaiiiitaminute" feeling, and starts digging through the data. Maybe they work there, maybe they don't, but they start piecing things together and prodding at the facade.
* Whispers start to echo across the blogosphere, and before too long somebody in the mainstream press with more of an investigative journalistic bent gets wind of it. They start doing research.
* LinkedIN memes and blog predictions, meanwhile, skyrocket.
* A news article or documentary effectively pointing out the lack of clothing on the supposed emporer's person comes out.
* Within days, the cards begin to collapse, and everybody in the tech industry nods along, as if they always suspected something from the beginning (even as they quietly mourn the deep losses they just took on their stock portfolio as the *wunderkind*'s company crumples).
* Fanbois continue to fanboi, of course.
...
#### AI-based startups that use AI to mimic humans are going to crater--hard (*p* 0.7)
...
[Werner Vogels thinks robots can help foster companionship](https://www.allthingsdistributed.com/2025/11/tech-predictions-for-2026-and-beyond.html); I deeply reject that premise, because humans have systematically rejected machines replacing humans in all but the most desperate of cases, and because cases where people have done so, [missed the mark](https://www.columbiapsychiatry.org/news/chatgpt-therapy-is-good-but-it-misses-what-makes-us-human) or [ended badly](https://www.cnn.com/2025/11/06/us/openai-chatgpt-suicide-lawsuit-invs-vis). Humans can still tell the difference between machines and humans, even if they can't say why they know. Worse, if humans *suspect* that the voice on the other end is a machine, they'll treat it wildly different.

But that said, lots of startups are going to go down this path, and while they might get some traction to start, they'll crater hard when they try to scale out. Because we are all of us lonely, and having an always-available companion seems like the easier solution than actually building empathy and connection with other humans. Certainly the more profitable one, anyway. But the results... [will not help people](https://www.youtube.com/watch?v=zkGk_A4noxI), only balance sheets.

#### OpenAI starts tying itself in knots to avoid having to make good on its promises (*p* 0.6)
...
Look, Sam knows how to project confidence--every CEO does at that level, it's a prerequisite of the job at this point--but that doesn't mean he knows how to actually bring it forth to fruition. We've all seen the [meme of the financial shell game](https://www.youtube.com/shorts/pB7faDHkHJI) that Microsoft, NVidia, OpenAI, and a few others are playing with "promises of investment", or perhaps you've watched the [More Perfect Union video](https://www.youtube.com/watch?v=l0K4XPu3Qhg). But Sam's playing at [an entirely new level](https://wlockett.medium.com/you-have-no-idea-how-screwed-openai-actually-is-8358dccfca1c) with [trillions on the line](https://www.wheresyoured.at/openai-onetrillion/), and you kinda have to wonder if he is resting much of his confidence on the idea that OpenAI is just "too big to fail" and can rely on government bailout in the event the piper does come calling.
...
#### "Vibe coding" is going to crater--hard (*p* 0.7)
...
* **"Software is eating the world."** --Marc Andreesen. *(Shouldn't come as a surprise to anyone, though at some point I hope that we realize that you can't eat software, and jobs like primary school education, plumbing, farming, and so on, are all pretty important to the continuation of Western society as we know it...)*
* **"The demand for software is infinite, yet its supply is finite."** --Neal Ford. To be precise, the demand for *new* software is infinite--it's not that everybody wants copies of Microsoft Word, it's that everybody has ideas of something that will make Word completely obsolete and hey, you're a software developer, right, you could code it for me and we could split the profits 70-30 my way because of course it's my idea.... 
* **Supplying new software requires some form of software development.** Canonically, this means hiring somebody who's accomplished with one or more programming languages. (We'll get to the alternatives in a moment, just hang on.)
* **Learning software development is a non-trivial investment of time and money.** Historically, this has meant going off to get a Computer Science degree from some college or university, which is often a four-year investment. Some folks can do it in less time, others do it in a self-taught manner, but it's not like you can read one "For Dummies" book and suddenly know all of what you need to know about how to build a mobile app, API backend, database storage, security, cloud management, and user interface design. This non-trivial investment is a core reason why the "supply of software (development) is finite".
...
#### AI coding assistant tooling is going to gain momentum (*p* 0.7)
... because AI's not gaining much steam anywhere else.

The aforementioned AI startups are constantly trying to sell us on AI in HR, medicine, and finance, but honestly, after a year or so, it doesn't seem to be having much effect, and companies are starting to backtrack and re-trench. For starters, companies tried to use AI agents to do hiring, and that landed... [poorly](https://www.buzzfeed.com/meganeliscomb/ai-job-interview-glitches-tiktok). Things really turned south when companies realized that candidates could do that too, and did. Maybe we can outsource other HR tasks... wait, [I think I've heard that one before](https://en.wikipedia.org/wiki/Up_in_the_Air_(2009_film)). 

Likewise, AI's use in the legal space has... [problems](https://www.npr.org/2025/07/10/nx-s1-5463512/ai-courts-lawyers-mypillow-fines). Sorry, Mike Lindell, you can't just hallucinate your way into proof of the 2020 stolen election! And he's [not](https://www.reuters.com/legal/government/judge-rebukes-minnesota-over-ai-errors-deepfakes-lawsuit-2025-01-13/) [alone](https://www.lawnext.com/2025/05/ai-hallucinations-strike-again-two-more-cases-where-lawyers-face-judicial-wrath-for-fake-citations.html).

Turns out, lots of people get grumpy when AI just makes stuff up out of nothing (even [people who are paid to do exactly that](https://cdt.org/insights/the-sag-aftra-strike-is-over-but-the-ai-fight-in-hollywood-is-just-beginning/)), and that trend is only likely to continue. Which then, if you're in the AI business, where can you let the non-deterministic nature of generative AI fly in a space that's open to it, yet structured enough to not be hamstrung by it?
...
#### Specification-oriented development will gain traction (*p* 0.6)
...
["Spec Kit makes your specification the center of your engineering process.](https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/) Instead of writing a spec and setting it aside, the spec drives the implementation, checklists, and task breakdowns.  Your primary role is to steer; the coding agent does the bulk of the writing." On the surface of this, I like where this is going, because elevating the abstractions in programming is something that I've been advocating for a very long time now. (We've been surfing at the "object" level for three decades--what say we start re-thinking the problem again?)

I'm not 100% convinced that this is going to be the "path forward" for our programming, but it's pretty clear that some kind of LLM-driven engagement, whether through coding agents as part of an IDE, or through these "specification" files, is going to markedly color much of what we programmers do for the next few years. I think there's a fair amount of rigor in what's being suggested here, but at the same time, this is [flying in the face of some pretty well-established doctrine](https://dsyme.net/2025/08/27/on-natural-language-programming/), and it's there's a pretty high mountain of evidence [against the idea of natural language](https://www.cs.utexas.edu/~EWD/transcriptions/EWD06xx/EWD667.html) as a deterministic way to write code.
...
#### Languages will matter less than fundamentals and concepts (*p* 0.6)
...
#### Micro-LLMs/Small Language Models (SLMs) gain traction for more widespread use (*p* 0.6)
...
#### We will see a new crop of languages trying to incorporate natural language (*p* 0.7)
...
#### Quantum will gain traction (*p* 0.5)
...
#### Android starts to move in on laptops (*p* 0.5)
...
The manufacturers feel it; it's why [Lenovo and Samsung have tablets that act like laptops](https://www.howtogeek.com/seen-a-samsung-tablet-lately-its-like-buying-a-pc/). Google, meanwhile, [already announced](https://www.androidauthority.com/chrome-os-becoming-android-3500661/) that they want to do away with ChromeOS in favor of Android, and the work over the last year pretty much demonstrates that they're ready to start backing that with laptop or desktop hardware.
...
#### I'll speak at a brand-new (to me) conference (*p* 0.4)
...
[^cloud]: Theoretically, anyway. We're starting to see that the cost of owning a virtual machine in the cloud isn't all that much better than it is to own it on-premises, which is leading a number of companies beyond a certain size to reconsider owning and operating their own data centers. It's still way better for smaller companies, where we can benefit from the margins that are gained from a big company running lots of servers, but at a certain size, it just doesn't make sense anymore.

[^substitute]: https://www.economicshelp.org/blog/glossary/substitute-goods/
